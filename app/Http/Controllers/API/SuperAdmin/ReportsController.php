...
class ReportsController extends Controller
{
            ...
            if($all->datafreq == 'Daily')
            {
                if(count($countries) > 0)
                {
                    $data = Registrations::select("reg_date as date", "serial_no as s.no", "candidates.passport_no as pp.no", "candidates.candidate_name as name", "eno.eno as e.no")
                                            ...
                                            ->where('center_id', $all->centreID->value)
                                            ->where('eno.centre_id', $all->centreID->value)
                                            ->whereIn('country', $countries)
                                            ->orderBy('serial_no',"ASC")
                                            ->get();
                }
            ...
            if($all->datafreq == 'Daily')
            {
                if(count($countries) > 0)
                {
                    $data = Registrations::select(DB::raw('SUBSTRING(lab_result.barcode,1,4) as code'),
                                            ...
                                            ->whereIn('country',$countries)
                                            ->where('center_id', $all->centreID->value)
                                            ->where('lab_result.centre_id', $all->centreID->value)
                                            ->orderBy('serial_no',"ASC")
                                            ->get();
                }
                else
            ...
            if($all->datafreq == 'Daily')
            {
                if(count($countries) > 0)
                {
                    $data = Registrations::select('lab_stickers.sticker_value_2 as code',
                                            ...
                                            ->whereIn('country',$countries)
                                            ->where('center_id', $all->centreID->value)
                                            ->where('centre_id', $all->centreID->value)
    ...
    public function export_report(request $request, $type)
    {
        ...
        $centre = Centres::find($all->centreID->value);
        ...
        else
        {
            ...
            foreach($all->keys as $key)
            {
                array_push($headings,strtoupper(str_replace('_',' ',$key->name)));
            }
                ...
                foreach ($all->keys as $key) {
                    if(isset($changeRow[$key->name]))
                    {
                        if (is_string($changeRow[$key->name])) {
                            $formattedRow[] = $enclosure . $changeRow[$key->name] . $enclosure;
                        } else if (is_date($changeRow[$key->name])) {
                            $formattedRow[] = $enclosure . date('Y-m-d', strtotime($changeRow[$key->name])) . $enclosure; // Format date
                        } else {
                            $formattedRow[] = $changeRow[$key->name];
                        }
                    }
                }
            ...
            {
                return response()->json(['success' => move_uploaded_file($filepath, 'storage/app/public/excel_exports/' . $filepath)], 500);
            }
        }
    }
